...
export default async function BlogDetailPage({ params }: Props) {
  ...
  const blogSchema = {
    ...
    author: {
      '@type': 'Person',
      name: post.author_name || 'Retreat Arcade',
    },
    publisher: {
      '@type': 'Organization',
      name: 'Retreat Arcade',
          ...
          {post.featured_image_url && (
            <div className="relative h-[500px] w-full overflow-hidden">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
                {post.category && <Badge className="bg-orange-600 hover:bg-orange-700 mb-4">{post.category}</Badge>}
                <h1 className="text-4xl md:text-5xl font-bold mb-4 leading-tight">{post.title}</h1>
                <div className="flex flex-wrap items-center gap-4 text-sm text-gray-200">
                  {post.author_name && <div className="flex items-center"><User className="h-4 w-4 mr-2" />{post.author_name}</div>}
                  {post.published_at && (
                  ...
                  {post.reading_time && <div className="flex items-center"><Clock className="h-4 w-4 mr-2" />{post.reading_time}</div>}
                </div>
              </div>
            </div>
          )}
                ...
                {contentImages.map((image: any, index: number) => (
                  <figure key={index} className="my-10">
                    <div className="relative overflow-hidden rounded-xl shadow-xl">
                      <img src={image.url} alt={image.alt || 'Content image'} className="w-full h-auto" />
                    </div>
                    {image.caption && <figcaption className="text-center text-sm text-gray-600 mt-3 italic">{image.caption}</figcaption>}
                  </figure>
                ))}
              </div>
            )}
