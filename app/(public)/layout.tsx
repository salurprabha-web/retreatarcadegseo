...
export default async function PublicLayout({
  ...
  const { data: settings, error } = await supabase.from('site_settings').select('*').limit(1).single<SiteSettings>();
     ...
     return (
        <div className="bg-brand-dark min-h-screen flex items-center justify-center text-center text-red-400 p-8">
            <div>
                ...
                <p className="text-gray-300">{error?.message || "Site settings could not be found."}</p>
