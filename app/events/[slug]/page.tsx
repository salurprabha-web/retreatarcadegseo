...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // Use meta columns if present, otherwise fallbacks
  const title = event.meta_title || event.title;
  const description = event.meta_description || event.summary || event.description?.slice(0, 160) || '';
...
export default async function EventDetailPage({ params }: Props) {
  ...
  // Build fallback schema if DB doesn't have it
  const domain = 'https://www.retreatarcade.in';
  const canonical = event.canonical_url || `${domain}/events/${event.slug}`;

  const schemaFromDb = event.schema_json && Object.keys(event.schema_json).length ? event.schema_json : null;
  ...
  const fallbackSchema = [
    {
      ...
      url: canonical,
      offers: event.price
        ? { '@type': 'Offer', price: String(event.price), priceCurrency: 'INR', url: canonical }
        : undefined,
    },
    {
      '@context': 'https://schema.org',
      ...
      image: event.image_url ? [event.image_url] : undefined,
      offers: event.price
        ? { '@type': 'Offer', price: String(event.price), priceCurrency: 'INR', url: canonical, availability: 'https://schema.org/InStock' }
        : undefined,
    },
  ];
      ...
      <div className="relative h-[60vh] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8">
          <div className="max-w-7xl mx-auto">
            <Link href="/events" className="inline-flex items-center text-cream-200 mb-4 hover:text-terracotta-400 transition-colors">
            ...
            <Card className="mb-8 bg-charcoal-900 border-terracotta-500/10">
              <CardContent className="pt-6">
                ...
                <div
                  ...
                  // description is HTML and will be rendered as-is
                ...
                <CardContent className="pt-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {galleryImages.map((image: string, index: number) => (
                      <div key={index} className="relative h-48 rounded-xl overflow-hidden border border-terracotta-500/20 hover:border-terracotta-500/40 transition-all">
                        <GalleryImage src={image} alt={`${event.title} gallery ${index + 1}`} className="w-full h-full object-cover hover:scale-110 transition-transform duration-300" />
                      </div>
                    ))}
                  </div>
                </CardContent>
              ...
              <div className="mt-16">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {similarEvents.map((ev) => (
                    <Card key={ev.id} className="overflow-hidden bg-charcoal-900 border-terracotta-500/20 hover:border-terracotta-500/40 transition-all">
                      <div className="relative h-48 overflow-hidden">
                        <img src={ev.image_url} alt={ev.title} className="w-full h-full object-cover hover:scale-110 transition-transform duration-500" />
                      </div>

                      <CardContent className="p-5 space-y-3">
                        <h3 className="text-xl font-semibold text-cream-50 line-clamp-2">{ev.title}</h3>

                        <p className="text-sm text-cream-300 line-clamp-2">{ev.summary}</p>

                        <div className="flex items-center text-sm text-cream-300">
                          <Calendar className="h-4 w-4 mr-2 text-terracotta-400" />
                          {ev.start_date ? new Date(ev.start_date).toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' }) : 'Date TBA'}
                        </div>
                      </CardContent>

                      <CardFooter>
                        <Button asChild className="w-full bg-terracotta-500 hover:bg-terracotta-600 text-white rounded-full">
            ...
            <Card className="sticky top-24 bg-charcoal-900 border-terracotta-500/20">
              <CardContent className="pt-6">
                <div className="space-y-4 mb-6">
                  ...
                  {event.location && (
                    <div className="flex items-center text-cream-300">
                      <div className="w-10 h-10 rounded-lg bg-cream-500/10 flex items-center justify-center mr-3">
                  ...
                  {event.max_participants && (
                    <div className="flex items-center text-cream-300">
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-cream-50">Up to {event.max_participants} guests</p>
                      </div>
                    </div>
                  )}
                </div>

                <Button asChild className="w-full bg-terracotta-500 hover:bg-terracotta-600 text-white rounded-full" size="lg">
                ...
                <div className="mt-6 p-4 bg-terracotta-500/10 border border-terracotta-500/20 rounded-xl">
                  <p className="text-sm text-cream-300 text-center">Contact us for pricing and customization options! 9063679687</p>
                </div>
              </CardContent>
            </Card>
          </div>
